...
model.eval()
cmnDf = pd.read_csv('insectNames.csv')
...
def evaluate(model, image, cmnDf):
    ...
    with torch.inference_mode():
        ...
        return sciPred, cmnPred


def get_prediction(PATH_TO_IMAGE):
    image = cv2.imread(PATH_TO_IMAGE)
    image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    sciPred, cmnPred = evaluate(model, image, cmnDf)
    return sciPred, cmnPred
...
def get_output():
    if request.method == 'POST':
        ...
        p,c = get_prediction(img_path)
        
        c = c.title()

    return render_template("index.html", prediction=p, cmnName = c, img_path=img_path)
